...
export default function Analysis() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-xl font-mono font-semibold">Run Analysis</h1>
        <p className="text-xs text-zinc-600 mt-0.5">
          Select SKUs and launch the multi-agent pipeline
        </p>
      </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
            ...
            <label className="flex items-center gap-2.5 cursor-pointer mb-3">
              <input
                ...
                className="w-3.5 h-3.5 accent-[#b5f23d]"
              />
            ...
            <button
              onClick={handleRun}
              disabled={!canRun}
              className={`w-full flex items-center justify-center gap-2 py-2.5 rounded font-mono text-sm font-medium transition-all ${
                canRun
                  ? "bg-[#b5f23d] text-zinc-950 hover:opacity-90"
              ...
              {result.recommendations?.length > 0 && (
                <div>
                  <p className="text-sm font-mono text-zinc-400 mb-3">
                    Recommendations ({result.recommendations.length} SKUs)
                  </p>
                  <InventoryTable
          ...
          {!running && !result && events.length === 0 && (
            <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-16 text-center">
              <div className="w-12 h-12 rounded-full bg-[#b5f23d]/10 border border-[#b5f23d]/20 flex items-center justify-center mx-auto mb-4">
                <Play size={20} className="text-[#b5f23d]" />
              </div>
              <p className="text-zinc-400 font-mono text-sm mb-1">
                Select SKUs and click Run Analysis
              </p>
              <p className="text-zinc-600 font-mono text-xs">
                The pipeline runs XGBoost forecasting, inventory optimization, RAG retrieval, and
