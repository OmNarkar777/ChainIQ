  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
        ...
        <KPI
          label="At Risk"
          value={analytics ? `${analytics.at_risk_pct}%` : null}
          icon={AlertTriangle}
        ...
        <KPI
          label="Avg Supply"
          value={analytics ? `${analytics.avg_days_of_supply}d` : null}
          icon={Clock}
        ...
        <KPI
          label="Inv. Value"
          value={analytics ? formatCurrency(analytics.total_inventory_value) : null}
          icon={Package}
          accent="text-[#b5f23d]"
          loading={analyticsLoading}
        ...
        <KPI
          label="Order Now"
          value={analytics ? formatCurrency(analytics.order_value_needed) : null}
          icon={DollarSign}
          accent="text-red-300"
          loading={analyticsLoading}
          sub="CRITICAL + HIGH value"
        />
      </div>
            ...
            <ResponsiveContainer width="100%" height={160}>
              <AreaChart data={analytics.demand_trend} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                ...
                <Area
                  ...
                  fill="url(#demandGrad)"
                  dot={false}
                  activeDot={{ r: 3, fill: "#b5f23d", strokeWidth: 0 }}
                />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-5">
          ...
          {analyticsLoading ? (
            <div className="space-y-2">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="skeleton h-6 rounded" style={{ width: `${80 - i * 12}%` }} />
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg">
          <div className="flex items-center justify-between px-5 py-3 border-b border-zinc-800">
            ...
            <Link
              to="/inventory"
              className="text-xs font-mono text-[#b5f23d] flex items-center gap-1 hover:opacity-80 transition-opacity"
            >
          ...
          {critical.length > 0 ? (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-zinc-600">
                    {s.days_until_stockout?.toFixed(1)}d left
            ...
            <div className="py-12 text-center">
              <p className="text-zinc-600 font-mono text-sm">All SKUs — inventory healthy</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg">
          ...
          {runs.length > 0 ? (
            runs.slice(0, 6).map((r) => (
              <div
                key={r.run_id}
                className="flex items-center justify-between px-5 py-3 border-b border-zinc-800/50 last:border-0"
              >
                <div>
                  <p className="text-xs font-mono text-zinc-400">#{r.run_id.slice(0, 8)}</p>
                  <p className="text-xs text-zinc-600">
                    {r.skus_analyzed} SKUs · {formatRunTime(r.created_at)}
                  </p>
                </div>
                <span
                  className={`text-xs font-mono px-2 py-0.5 rounded border ${
                    r.status === "DONE"
                      ? "text-[#b5f23d] border-[#b5f23d]/30 bg-[#b5f23d]/10"
            ...
            <div className="py-12 text-center">
              ...
              <Link
                to="/analysis"
                className="inline-flex items-center gap-1.5 text-xs font-mono text-[#b5f23d] border border-[#b5f23d]/30 px-3 py-1.5 rounded hover:bg-[#b5f23d]/10 transition-colors"
              >
